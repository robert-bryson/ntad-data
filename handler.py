...
def run(event, context):
    '''
    Handler for agol_driver.  Calls both shp2agol and agol2s3 depending on event payload
    '''
    ...
    # Login to AGOL
    gis = GIS(os.environ.get('AGOL_URL'),secret['username'],secret['password'])

    # Get all content from AGOL within MRDS group or under specific user
    agol_fs = gis.content.search(query=f'owner:{gis.properties.user.username} type:feature service')

    # init s3
    s3 = boto3.resource('s3')

    # Get all content from feature service json
    fs_conf = s3.meta.client.get_object(Bucket=event['s3_bucket'],Key=event['fs_conf_path'])
    ...
    if agol_fs:
        ...
        for fs in agol_fs:
            if fs.id not in [x['fs_id'] for x in fs_rec_json]:
                ...
                print(f"New feature service found in AGOL that is absent from conf file. Added {fs.title} - {fs.id}")
        ...
        for fs_rec in fs_rec_json:
            if fs_rec['fs_id'] not in [x.id for x in agol_fs]:
                print(f"Feature service listed in conf file not found in AGOL, will be deleted from conf file. {fs_rec['fs_name']} - {fs_rec['fs_id']}")
        fs_rec_json = [x for x in fs_rec_json if x['fs_id'] in [fs.id for fs in agol_fs]]
        ...
        if event['method'] == "shp2agol":
            ...
            for update_fs in event['data']:
                ...
                if update_fs['fs_name'] in [x.title for x in agol_fs] and not update_fs['fs_id']:
                    raise ValueError(f"Feature service title '{update_fs['fs_name']}' exists in AGOL but no item id was provided in the event payload to overwrite")

                # Check if fs_id provided in event payload is a current feature service
                if update_fs['fs_id'] and update_fs['fs_id'] not in [x.id for x in agol_fs]:
                    raise ValueError(f"Feature service '{update_fs['fs_name']}' has a specified id that is not currently in AGOL, check feature service id in event payload")
            ...
            for export_fs in event['data']:
                if export_fs['fs_id'] not in [x.id for x in agol_fs]:
                    raise ValueError(f"Feature service to be exported '{export_fs['fs_name']}' has a specified id that is not currently in AGOL, check feature service id in event payload")
...
def shp2agol(gis:GIS, s3, event):
    ...
    for add_service in event["data"]:
        ...
        objs = list(bucket.objects.filter(Prefix=event['shp_s3_path'] + add_service['shp_name'] + add_service['format']))
        if not objs:
            print(f"Key not found in s3 bucket, skipping {event['shp_s3_path'] + add_service['shp_name'] + add_service['format']}")
            continue

        # Get data from s3 bucket
        download_path = f"{tempfile.gettempdir()}/{add_service['fs_name']}"
        try:
            s3.meta.client.download_file(event['s3_bucket'], event['shp_s3_path'] + add_service['shp_name'] + add_service['format'], download_path)
        except:
            print("Unable to download file from s3")
            raise

        # Set content props for AGOL item
        shp_props = {"type":'Shapefile', "title":add_service['fs_name'],"overwrite":True}
        ...
        if 'fs_id' in add_service.keys() and add_service['fs_id']:
            try:
                ...
                sys.exit(f"Incorrect feature service ID provided in event ({add_service['fs_id']}) - Feature service not found in content of user {gis.properties.user.username}")
        else:
            print(f"{add_service['fs_name']} is a new feature service and will be added to AGOL")
        ...
        try:
            ...
            capabilities = event['fs_capabilities'] if event['fs_capabilities'] else "Query, Editing, Create, Update, Delete, ChangeTracking, Extract"
            service_props = {
            "hasStaticData":False,
            ...
            if resp is not True:
                raise Exception('Failed to add published feature service to existing MRDS webmap')
            else:
    ...
    with tempfile.TemporaryFile('w+', delete=False) as tmpfile:
        ...
        tmpfile.close()
        s3.meta.client.upload_file(tmpfile.name, event['s3_bucket'],event['fs_conf_path'])
...
def agol2s3(gis, s3, event):
    ...
    for export_fs in event['data']:
        ...
        exported_shp = feature_service.export(export_fs['shp_name'],event['export_format'])
        print('exported feature service')

        # Download exported shapefile then delete shapefile in AGOL
        shp_path = f"{tempfile.gettempdir()}/{export_fs['shp_name'] + export_fs['format']}"
        if exists(shp_path):
            os.remove(shp_path)
        exported_shp.download(tempfile.gettempdir(),export_fs['shp_name'] + export_fs['format'])
        exported_shp.delete()
        print('downloaded shp and deleted shp object')

        # Upload and overwrite new shapefile to s3 bucket
        s3.meta.client.upload_file(shp_path, event['s3_bucket'], event['target_s3_filepath'] + export_fs['shp_name'] + export_fs['format'])
